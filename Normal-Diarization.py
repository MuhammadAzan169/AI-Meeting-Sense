...
WITH AUTOMATIC SPEAKER IDENTIFICATION FROM VOICE SAMPLES
ENHANCED VERSION WITH DOMAIN ADAPTATION AND PDF GENERATION
"""
import os
import json
import torch
...
class Config:
    ...
    def get_voice_samples_dir():
        ...
        voice_samples_dir = os.path.join(base_dir, "voices")
        
        # Create directory if it doesn't exist
        if not os.path.exists(voice_samples_dir):
            os.makedirs(voice_samples_dir, exist_ok=True)
            print(f"📁 Created voice samples directory: {voice_samples_dir}")
...
class MeetingTranscriptPDF:
    ...
    def _register_fonts(self):
        ...
        try:
            ...
            # Use default fonts if custom registration fails
    
    def _define_custom_styles(self):
        """Define professional color theme and styles"""
        # Color theme: Dark Blue, Light Blue, and White
        self.dark_blue = colors.HexColor('#1E3A8A')  # Dark Blue
        self.light_blue = colors.HexColor('#60A5FA')  # Light Blue
        ...
        # Section Header Style - REMOVED BORDER/OUTLINE as requested
        self.styles.add(ParagraphStyle(
            name='SectionHeader',
            parent=self.styles['Heading2'],
            fontSize=16,
            textColor=self.dark_blue,
            alignment=TA_CENTER,  # Changed to CENTER as requested
            spaceAfter=12,
            spaceBefore=20,
            fontName='Arial-Bold',
            # Removed all border properties as requested
            # No borderPadding, borderColor, borderWidth, borderRadius
            backgroundColor=self.light_blue  # Keeping background color only
        ))
    ...
    def create_pdf(self, transcript_data, output_path=None, 
        ...
        # 3. Transcript Table Header - Now center-aligned as requested
        ...
        # 5. Create the table
        # Column widths: Speaker 1.2", Start 0.8", End 0.8", Transcript 4.2"
        col_widths = [1.2*inch, 0.8*inch, 0.8*inch, 4.2*inch]
        transcript_table = Table(table_data, colWidths=col_widths, repeatRows=1)
        
        # Apply table style - CENTER the table on the page as requested
        transcript_table.setStyle(TableStyle([
            ...
            ('ALIGN', (0, 1), (2, -1), 'CENTER'),  # Center align first 3 columns
            ('ALIGN', (3, 1), (3, -1), 'LEFT'),     # Left align transcript
            ('VALIGN', (0, 0), (-1, -1), 'TOP'),
            ...
            # CENTER the entire table on the page as requested
...
def preprocess_audio_for_speaker_id(waveform, sr, target_sr=16000):
    ...
    # Normalize volume (RMS normalization)
...
def load_embedding_model():
    ...
    model_sources = [
        "speechbrain/spkrec-ecapa-voxceleb",  # Alternative model
    ...
    for source in model_sources:
        try:
            ...
            # Use model cache directory for saving
    ...
    # If all online models fail, try to use a local model if available
    print("\n⚠ All online models failed. Trying local model...")
    try:
        # Create a minimal local model structure
        ...
        # Try to load any existing local model
        model = SpeakerRecognition.from_hparams(
    ...
    for filename in os.listdir(samples_dir):
        if filename.lower().endswith(('.mp3', '.wav', '.m4a', '.flac')):
            ...
            try:
                ...
                # SIMPLIFIED preprocessing - avoid complex operations that trigger compute_features
                if waveform.dim() > 1 and waveform.shape[0] > 1:
                    waveform = waveform.mean(dim=0, keepdim=True)
                ...
                # Ensure minimum length and take a 3-second chunk from the middle
                ...
                # Take a 3-second chunk from the middle for consistency
                start_sample = max(0, (waveform.shape[1] - min_samples) // 2)
                end_sample = start_sample + min_samples
                chunk = waveform[:, start_sample:end_sample]
                
                # Use the model's encode_batch directly without additional preprocessing
                with torch.no_grad():
                    emb = model.encode_batch(chunk)
                    if isinstance(emb, tuple):
                        emb = emb[0]  # Take the first element if it returns a tuple
...
# ---------------------------------------------------
def identify_speaker(segment_embedding, voice_embeddings, min_threshold=0.0):
    """
    Compare segment embedding with all voice sample embeddings.
    Uses normalized scores and relative ranking.
    """
...
def segment_audio_for_speaker_id(wav_path, segment_duration=3.0, overlap=1.0):
    ...
    # Apply preprocessing for speaker ID only
...
def extract_embeddings(model, segments):
    ...
    for i, segment in enumerate(segments):
        try:
            ...
            # Add a zero vector as placeholder (will be filtered out later)
            if len(embeddings) > 0:
                embeddings.append(np.zeros_like(embeddings[0]))
            else:
                # If no successful embeddings yet, create a dummy one
                embeddings.append(np.zeros(192))  # ECAPA typically has 192-dim embeddings
...
def estimate_speakers(embeddings, max_speakers=8):
    ...
    for emb in embeddings:
        if np.any(emb):  # Check if not all zeros
...
def transcribe_segment(model, audio_path, start, end, sr):
    ...
    """
    # Load original audio without any preprocessing
    waveform, original_sr = torchaudio.load(audio_path)
    
    # Convert timestamps from preprocessed SR to original SR if needed
    if original_sr != sr:
        # Scale timestamps to original sample rate
    ...
    # Save with original sample rate for best quality
    sf.write(temp_file, audio_np, original_sr, subtype='PCM_16')
    
    try:
        # Transcribe using original, unprocessed audio
...
def main(audio_path, samples_dir=None):
    ...
    # Load voice samples with enhanced preprocessing (for speaker ID only)
    ...
    # Segment with preprocessing FOR SPEAKER ID ONLY
    ...
    for spk, start, end in filtered_segments:
        if spk not in speaker_map:
            # Auto-identify speaker using voice samples
            ...
            if cluster_embedding is not None and voice_embeddings:
                ...
                # AUTO-ASSIGN: Use best match without asking for confirmation
                speaker_map[spk] = identified_name
                print(f"✅ Auto-assigned: {identified_name} (score: {score:.4f})")
            else:
                # Fallback: assign generic name if no voice samples available
    ...
    # Save JSON transcript to output directory
    output_dir = Config.get_output_dir()
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    # AUTOMATICALLY GENERATE PDF (no prompts)
    if results:
        # Generate meeting title from filename
        base_name = os.path.splitext(os.path.basename(audio_path))[0]
        meeting_title = f"Meeting: {base_name.replace('_', ' ').title()}"
        
        # Create PDF with table layout
        ...
        pdf_path = pdf_generator.create_pdf(
            ...
            meeting_date=datetime.now().strftime("%B %d, %Y at %I:%M %p"),
            agenda_items=None  # Can add agenda items if needed
        )
        ...
        if pdf_path:
            ...
            print(f"     - {json_path}")
            print(f"     - {pdf_path} (with 4-column table layout)")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("🎙️  LIVE MEETING DIARIZATION WITH SPEAKER ID")
    print("="*50 + "\n")
    ...
    if choice == "1":
        ...
        try:
            # AUTOMATICALLY process with auto-identification and PDF generation
            print("\n🔧 Settings: Auto-speaker identification | Automatic PDF generation")
